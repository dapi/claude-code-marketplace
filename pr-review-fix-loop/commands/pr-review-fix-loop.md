...
Перед запуском loop создать файл `.pr-review-loop-report.md` с заголовком:
...
`Шаг 3.5: Запустить direnv exec . bundle exec rubocop -a для изменённых файлов. Если rubocop изменил файлы, дописать это в .pr-review-loop-report.md. `
...
Прочитать `.pr-review-loop-report.md` и определить причину завершения по маркерам:
...
Файл `.pr-review-loop-report.md` НЕ удалять — это артефакт для пользователя.
