...
### 3. Коммит и push (только при REVIEW CLEAN)

Коммиты создаются внутри каждой итерации (шаг 4.5 в промпте). После завершения loop:

1. Проверить остались ли незакоммиченные изменения:
...
2. Push (только при REVIEW CLEAN):
   ...
   - Сообщить пользователю что изменения запушены
