...
# Assemble prompt
PROMPT="КАЖДУЮ итерацию ВСЕГДА выполняй ВСЕ шаги по порядку. НЕ пропускай шаги, даже если в прошлой итерации ты уже исправил issues. ШАГИ -- ${CODEX_BG_STEP}Шаг 0.5 - ЖУРНАЛ РЕШЕНИЙ: Прочитать файл .claude/pr-review-loop-report.local.md и найти все секции USER DECISION. Каждое решение пользователя ФИНАЛЬНОЕ и является его требованием. Составить список всех решений: для каждого запомнить файл, тему и выбор пользователя. Этот список использовать при фильтрации issues в Шаге 2. Шаг 1: Запустить /pr-review-toolkit:review-pr $ASPECTS и дождаться результата. ${CODEX_COLLECT_STEP}Шаг 2: Собрать все issues. Из review-pr взять issues с пометкой review-pr. ${CODEX_REPORT_NOTE}Отфильтровать только issues с criticality от $MIN_CRITICALITY из 10 и выше. Issues с criticality ниже $MIN_CRITICALITY игнорировать. Затем сверить оставшиеся issues со списком решений из Шага 0.5: если issue касается того же файла и той же темы по которой пользователь уже принял решение - ИСКЛЮЧИТЬ этот issue из списка и НЕ считать в issues_count. В секции итерации пометить такие issues как SKIPPED с ссылкой на решение пользователя. Добавить в файл .claude/pr-review-loop-report.local.md секцию текущей итерации - номер итерации, количество найденных issues выше порога, для каждого issue его источник и criticality и краткое описание с указанием файла. Шаг 2.5: Записать статистику итерации командой ${PLUGIN_ROOT}/scripts/record-iteration.sh N K где N это номер текущей итерации и K это количество issues выше порога найденных в шаге 2. Шаг 3a: Если найдены issues выше порога - сгруппировать их по затронутому файлу или области кода. Для каждой уникальной группы запустить агент feature-dev:code-explorer через Task tool с задачей проанализировать архитектуру этой области кода, найти похожие реализации и паттерны, вернуть список ключевых файлов. Запускать explorer-агентов параллельно. Дождаться завершения всех. Шаг 3b: Прочитать ключевые файлы возвращённые каждым explorer-агентом для понимания контекста и паттернов. Дописать в .claude/pr-review-loop-report.local.md секцию EXPLORATION с результатами - область, найденные паттерны, ключевые файлы."
...
PROMPT="$PROMPT Шаг 3d: Для каждого issue с criticality ниже 7 но выше порога - прочитать файл и строку, понять причину используя контекст из exploration, исправить минимально и точечно. НЕ рефакторить окружающий код, НЕ добавлять комментарии и docstrings, НЕ делать косметических правок. Шаг 3e: После всех исправлений дописать в .claude/pr-review-loop-report.local.md что было исправлено - для TDD-issues указать имя spec-файла и результаты red-green, для остальных краткое описание фикса. ЗАПИСЬ РЕШЕНИЙ: Если в ходе этой итерации пользователь принимал решение - через AskUserQuestion, через текстовый ответ, через подтверждение или отклонение предложения - НЕМЕДЛЕННО дописать в .claude/pr-review-loop-report.local.md блок в формате: USER DECISION iteration=N -- file: путь к файлу -- topic: о чём было решение (кратко) -- choice: что именно выбрал пользователь -- context: почему этот вопрос возник. Каждое решение записывать отдельным блоком. Это ОБЯЗАТЕЛЬНО для любого выбора пользователя без исключений. ${LINT_STEP}"
...
PROMPT="$PROMPT Шаг 4.5 - КОММИТ: Если были исправления в этой итерации - закоммитить их. Получить список изменённых файлов через ${CMD_PREFIX}git diff --name-only и ${CMD_PREFIX}git diff --cached --name-only. Из списка ИСКЛЮЧИТЬ файлы *.local.md и .gitignore (если .gitignore изменён только добавлением правила .claude/*.local.md - включить его в первый коммит). Добавить файлы через ${CMD_PREFIX}git add для каждого файла. Создать коммит с сообщением fix: address PR review issues (iteration N) и Co-Authored-By: Claude Opus 4.6 <noreply@anthropic.com>. Если pre-commit hook падает - записать в отчёт что коммит не удался и продолжить. НЕ пушить."

PROMPT="$PROMPT Шаг 5 - РЕШЕНИЕ И СТАТУС: Дописать в .claude/pr-review-loop-report.local.md маркер ITERATION N COMPLETED issues_count=K где K это количество issues выше порога найденных в шаге 2 текущей итерации. Затем прочитать файл .claude/pr-review-loop-report.local.md и извлечь значения K из всех строк вида ITERATION M COMPLETED issues_count=K. Построить массив counts по порядку итераций. РЕШЕНИЕ -- Вариант А ЧИСТО: Если K текущей итерации равно 0 - дописать статус ЧИСТО, добавить секцию ИТОГО с общим количеством итераций и исправленных issues, вывести <promise>REVIEW CLEAN</promise>. Вариант Б СТАГНАЦИЯ: Если массив counts содержит 5 или более значений И значение последнего элемента больше или равно значению элемента на позиции 5 с конца - дописать статус СТАГНАЦИЯ с трендом последних 5 значений, вывести <promise>REVIEW STAGNANT</promise>. Вариант В ПРОДОЛЖИТЬ: Иначе дописать статус ПРОДОЛЖИТЬ с количеством исправленных issues и завершить итерацию БЕЗ promise, loop продолжится."
