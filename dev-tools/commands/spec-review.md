...
```
VERSION = "1.4.0"                # Версия команды (синхронизирована с frontmatter)
...
#### Обязательные агенты (всегда запускать — 7 штук):
...
#### Опциональный агент: spec-ux (если есть UI)

**Запускать ТОЛЬКО если спецификация содержит UI:**
- Описание экранов, страниц, компонентов
- User flows с интерфейсом
...
**Минимум 7 агентов, максимум 8 (с spec-ux).**

---

## Фаза 3: Объединение результатов

После получения JSON от всех субагентов (7-8):
...
- [ ] Запущены 7 обязательных субагентов ПАРАЛЛЕЛЬНО (в одном сообщении)
- [ ] Определено, нужен ли spec-ux (есть ли UI в спеке)
- [ ] Если есть UI — запущен spec-ux (итого 8 агентов)
