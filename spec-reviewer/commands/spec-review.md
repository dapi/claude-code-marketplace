...
| **Docmost page** | MCP: `mcp__docmost__docmost-get_page` (primary), fallback `mcp__docmost__docmost-search` |
| **Локальный файл** | Tool: `Read` |

**Аргумент команды:** `$ARGUMENTS`
- Если пусто — спроси: "Укажи ссылку на Google Doc / Docmost, номер GitHub issue или путь к файлу"
...
- Docmost:
  - если URL на известном Docmost-домене и содержит `/p/{pageId}` или `/pages/{pageId}` → извлечь `{pageId}` и вызвать `mcp__docmost__docmost-get_page`
  - если `pageId` не извлекается → попытаться найти страницу через `mcp__docmost__docmost-search` (по slug/title из URL), затем `mcp__docmost__docmost-get_page`
  - если тип URL сомнителен → открыть ссылку и проверить, что это Docmost, после чего читать через MCP Docmost
...
3. **🧩 Свое решение**
   → Пользователь формулирует кастомное решение
   → Зафиксировать rationale в спецификации/отчёте
   → Сопоставить с одним из статусов: fixed | deferred | reclassified | rejected
...
1. ** Обработать оставшиеся индивидуально**
   ...
   → Для каждой: принять/отложить/переклассифицировать/отклонить/кастомно
...
4. **🧩 Своя стратегия обработки**
   → Пользователь задаёт правила обработки
   → Для каждого замечания выполнить маппинг в fixed/deferred/reclassified/rejected
   → После обработки — снова Gate Check
...
4. **🧩 Свое решение**
   → Запросить желаемое действие
   → Добавить rationale в комментарий
   → Сопоставить с fixed/deferred/reclassified/rejected
...
| Docmost страница не найдена | Ошибка: "❌ Docmost страница не найдена. Проверьте ссылку и доступ." |
| Не удалось определить pageId в Docmost URL | Fallback: `docmost-search` по slug/title; если не найдено — ошибка с просьбой дать прямой URL страницы |
...
**Чтение (предпочтительно для Docmost ссылок):**
```
mcp__docmost__docmost-get_page
  pageId: "{PAGE_ID}"
```

**Fallback-поиск по slug/title из URL:**
```
mcp__docmost__docmost-search
  query: "{slug_or_title}"
```

**Обновление страницы (если нужно записать отметку):**
```
mcp__docmost__docmost-update_page
  pageId: "{PAGE_ID}"
