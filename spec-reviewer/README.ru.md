...
2. В шагах аппрува/gate-check выбирай `custom` (своя стратегия) и задавай политику:
...
- Добавь правило "повторять до critical/high/medium/low = 0".
- Если достигнут лимит итераций в одном запуске — запусти команду снова с той же политикой.

Что значит автопилот здесь:
- автоматическая обработка замечаний + повторный анализ;
- но не безусловный автопримен всех изменений без политики.
